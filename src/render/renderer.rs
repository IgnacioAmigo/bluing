use super::{
    buffer::VertexBuffer,
    buffer::{ElementBuffer, VertexArray},
    data::AttributedVertex,
...
impl SpriteRenderer {
    pub fn from_res(
        res: &Resources,
        screen_dimensions: glm::Vec2,
    ) -> Result<SpriteRenderer, String> {
        let vertices: Vec<SpriteVertex> = vec![
            SpriteVertex {
                postex: glm::vec4(0.0, 0.0, 0.0, 0.0),
            }, // top
            SpriteVertex {
                postex: glm::vec4(0.0, 1.0, 0.0, 1.0),
            }, // bottom right
            SpriteVertex {
                postex: glm::vec4(1.0, 0.0, 1.0, 0.0),
            }, // bottom left
            SpriteVertex {
                postex: glm::vec4(0.0, 1.0, 0.0, 1.0),
            },
            SpriteVertex {
                postex: glm::vec4(1.0, 1.0, 1.0, 1.0),
            },
            SpriteVertex {
                postex: glm::vec4(1.0, 0.0, 1.0, 0.0),
            },
    ...
    pub fn draw_subtexture(&self, subtexture: &Subtexture, position: glm::Vec2) {
        self.draw_quad(
            ...
            3.0,
            subtexture.get_normalized_rect(),
        );
    }
    ...
    pub fn draw_quad(
        ...
        let model = glm::translate(
            &model,
            &glm::vec3(
                texture.width_f() * scale * 0.5,
                texture.height_f() * scale * 0.5,
                0.0,
            ),
        );
        ...
        let model = glm::scale(
            &model,
            &glm::vec3(
                texture.width_f() * scale * (sub_tex_rect.z),
        ...
        self.program
            .set_vector4f("subTexCoords\0".as_ptr(), sub_tex_rect);
    ...
    pub fn draw_rect(&self, rect: glm::Vec4, color: glm::Vec3) {
        ...
        let model = glm::scale(&model, &glm::vec3(rect.z, rect.w, 0.0));
    ...
    pub fn draw_circle(&self, rect: glm::Vec4, _color: glm::Vec3) {
        let model = glm::Mat4::identity();
        let model = glm::translate(&model, &glm::vec3(rect.x, rect.y, 0.0));
